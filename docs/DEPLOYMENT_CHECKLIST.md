# 备份与容灾服务 上线部署检查清单（DEPLOYMENT_CHECKLIST）
...
- [ ] ConfigMap 环境变量正确（SM_ENV / SM_DATABASE_NAME=sm_backup_dr）
...
1. `helm lint helm/sm-backup-dr`
2. `helm upgrade --install sm-backup-dr helm/sm-backup-dr -n sm-prod -f helm/sm-backup-dr/values-prod.yaml`
3. 等待 rollout：`kubectl -n sm-prod rollout status deploy/sm-backup-dr`
